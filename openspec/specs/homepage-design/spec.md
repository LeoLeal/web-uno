...
### Requirement: Logo displays "P2P UN" with card as "O"

The homepage logo SHALL display "P2P UN" in Playfair Display font (cream color #f5e6c8) with the letter "O" replaced by a tilted wild card. The card MUST be rotated approximately 15 degrees clockwise and include a drop shadow for depth.

#### Scenario: Logo renders correctly on desktop

- **WHEN** user views the homepage on a screen wider than 640px
- **THEN** the logo displays "P2P UN" with the wild card tilted as the "O", with visible drop shadow
