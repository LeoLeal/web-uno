...
- **Host can randomize player order** via a "Randomize player order" button next to the player count in the lobby header.
- **PlayerList switches from CSS Grid to flex-wrap** layout to support drag-and-drop reordering with `@dnd-kit`.
...
- `lobby-management`: Player list no longer alphabetically sorted; uses explicit `playerOrder` from Yjs. PlayerList layout changes from CSS grid to flex-wrap.
...
- **Components**: `PlayerList.tsx` (flex-wrap + @dnd-kit sortable + drag handle), `OpponentIndicator.tsx` (number badge), `PlayerHand.tsx` (player number label), `OpponentRow.tsx` / `GameBoard.tsx` (pass player numbers through).
