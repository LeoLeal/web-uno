...
- Implement wild card rendering via inlined SVGs: wild card designs embedded as React components with CSS classes on paths, enabling grayscale-except-chosen-color effect on discard pile
...
- `gameplay-wild-cards`: Wild card color selection modal, inlined SVG-based wild card rendering with CSS color classes, Wild Draw Four effects
...
- **Components**: `PlayerHand` gains card click handlers; `DeckPile` gains click handler; new `WildColorModal` component; `DiscardPile` renders played wilds via inlined SVG; `OpponentIndicator` gains UNO balloon
- **SVG Assets**: Wild card SVG paths inlined in `WildCardSvg` React component using CSS classes on color paths instead of inline styles
